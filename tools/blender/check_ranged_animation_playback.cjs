// Read-only browser playback check for the isolated study page and its four videos.
const { chromium } = require('playwright');
const fs = require('node:fs');
const path = require('node:path');
const { pathToFileURL } = require('node:url');
const root = path.resolve(__dirname, '../..');
const stage = path.join(root, 'art/blender/candidates/animation-v3/ranged-family');
...
(async () => {
  ...
  try {
    ...
    if (errors.length || videos.length !== 4 || videos.some(video => video.error || video.paused || video.presented < 20 || video.wraps < 1 || Math.abs(video.duration - 2.52) > 0.001)) {
