...
def main():
    ...
    if args.preview:
        board=REVIEW.backdrop((1024,870));draw=ImageDraw.Draw(board)
        for row,unit in enumerate(['archer','slinger','grenadier']):
    ...
    summary={};sections=[]
    for unit in ['archer','slinger','grenadier']:
    ...
    (STAGE/'review.html').write_text('''<!doctype html><html lang="de"><meta charset="utf-8"><title>AgeOfMax Bogen- und Wurfstudie</title>
...
<main><h1>Bogen und Wurf: 8 → 16 echte Blender-Frames</h1><p>Links aktueller Stand, rechts isolierte Studie. Drei Gangzyklen à 520 ms und drei Angriffe à 320 ms; Kontakt nach 160 ms. Der Bogen spannt Holz und Sehne, die Schleuder öffnet ihre freie Schnur, und der Grenadier zieht den Stift und wirft über Kopf. Pfeil, Stein und Granate verschwinden exakt im Freigabebild 12; die Waffenanker stammen weiter aus der echten deformierten Geometrie. Produktionsstand unverändert.</p>'''+''.join(sections)+'''
