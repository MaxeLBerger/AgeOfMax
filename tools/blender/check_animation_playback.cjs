// Read-only browser playback check for the isolated study page and its two videos.
const { chromium } = require('playwright');
const fs = require('node:fs');
const path = require('node:path');
const { pathToFileURL } = require('node:url');
const root = path.resolve(__dirname, '../..');
const stage = path.join(root, 'art/blender/candidates/animation-v3');
...
(async () => {
  ...
  try {
    ...
    if (errors.length || videos.length !== 2 || videos.some(video => video.error || video.paused || video.presented < 20 || video.wraps < 1 || Math.abs(video.duration - 2.52) > 0.001)) {
      throw new Error(JSON.stringify({ errors, videos }));
    }
    // Also seek both streams to the actual160ms contact of their first attack.
    await page.evaluate(async () => {
