"""Verify the existing static exporter on an isolated Renaissance scene copy.

The input scene is read only. Both copied .blend and rendered PNG stay in outdir.
Example: blender -b --python audit_renaissance_export.py -- --source candidate.blend
"""
import argparse
import hashlib
...
import refine_renaissance


def signature():
    result={}
    for obj in bpy.data.objects:
        if not obj.name.startswith('REN_'):
            continue
...
def main():
    ...
    parser.add_argument('--outdir',type=Path,default=ROOT/'art/blender/candidates/renaissance-v2/export-validation')
    ...
    public=ROOT/'public/assets/reborn/backgrounds/renaissance.png'
    public_hash=hashlib.sha256(public.read_bytes()).hexdigest()
    bpy.ops.wm.open_mainfile(filepath=str(source),load_ui=False)
    scene=bpy.context.scene
    if not bpy.data.collections.get('REN_Architecture'):
    ...
    scene.render.filepath=str(outdir/'exports/backgrounds/renaissance.png')
    prepared=copies/'background-renaissance.blend'
    export_scenes.save_scene(prepared)
    bpy.context.view_layer.update()
    geometry_before=signature()
    camera_before=refine_renaissance.camera_contract()
    terrain_before=refine_renaissance.lane_hash()
    ...
    export_scenes.export_static('backgrounds','renaissance','player',args.samples)
    bpy.context.view_layer.update()
    checks={'architecture_geometry_unchanged':geometry_before==signature(),
            'camera_unchanged':camera_before==refine_renaissance.camera_contract(),
            'terrain_unchanged':terrain_before==refine_renaissance.lane_hash(),
            ...
            'archive_hidden':bpy.data.collections['REN_SourceArchive'].hide_render,
    ...
    report={'status':'passed','input':str(source),'input_sha256':original_hash,
            'input_is_canonical':source==ROOT/'art/blender/background-renaissance.blend',
            'isolated_source':str(prepared),'exporter':'tools/blender/export_scenes.py::export_static',
            'output':str(outdir/'exports/backgrounds/renaissance.png'),
    ...
    print('RENAISSANCE_EXPORT_VALIDATED '+json.dumps(checks))
