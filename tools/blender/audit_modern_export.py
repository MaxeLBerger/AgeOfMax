"""Verify the existing static exporter on an isolated Modern scene copy.

The input scene is read only. Both copied .blend and rendered PNG stay in outdir.
Example: blender -b --python audit_modern_export.py -- --source candidate.blend
"""
import argparse
import hashlib
...
import refine_modern


def signature():
    result={}
    for obj in bpy.data.objects:
        if not obj.name.startswith('MOD_'):
            continue
...
def main():
    ...
    parser.add_argument('--outdir',type=Path,default=ROOT/'art/blender/candidates/modern-v2/export-validation')
    ...
    if not bpy.data.collections.get('MOD_Architecture'):
    ...
    scene.render.filepath=str(outdir/'exports/backgrounds/modern.png')
    prepared=copies/'background-modern.blend'
    export_scenes.save_scene(prepared)
    bpy.context.view_layer.update()
    geometry_before=signature()
    camera_before=refine_modern.camera_contract()
    terrain_before=refine_modern.lane_hash()
    ...
    export_scenes.export_static('backgrounds','modern','player',args.samples)
    bpy.context.view_layer.update()
    checks={'architecture_geometry_unchanged':geometry_before==signature(),
            'camera_unchanged':camera_before==refine_modern.camera_contract(),
            'terrain_unchanged':terrain_before==refine_modern.lane_hash(),
            ...
            'archive_hidden':bpy.data.collections['MOD_SourceArchive'].hide_render,
    ...
    report={'status':'passed','input':str(source),'input_sha256':original_hash,
            'input_is_canonical':source==ROOT/'art/blender/background-modern.blend',
            'isolated_source':str(prepared),'exporter':'tools/blender/export_scenes.py::export_static',
            'output':str(outdir/'exports/backgrounds/modern.png'),
    ...
    print('MODERN_EXPORT_VALIDATED '+json.dumps(checks))
