"""Verify the existing static exporter on an isolated Castle scene copy.

The input scene is read only. Both copied .blend and rendered PNG stay in outdir.
Example: blender -b --python audit_castle_export.py -- --source candidate.blend
"""
import argparse
import hashlib
...
import refine_castle


def signature():
    result={}
    for obj in bpy.data.objects:
        if not obj.name.startswith('CAS_'):
            continue
...
def main():
    ...
    parser.add_argument('--outdir',type=Path,default=ROOT/'art/blender/candidates/castle-v2/export-validation')
    ...
    if not bpy.data.collections.get('CAS_Architecture'):
        raise RuntimeError('The selected scene does not contain the refined architecture.')
    scene['authored_environment_revision']=2
    scene['environment_id']='castle'
    scene['runtime_background']='backgrounds/castle.png'
    scene['authoring_source']='tools/blender/refine_castle.py'
    ...
    scene.render.filepath=str(outdir/'exports/backgrounds/castle.png')
    prepared=copies/'background-castle.blend'
    export_scenes.save_scene(prepared)
    bpy.context.view_layer.update()
    geometry_before=signature()
    camera_before=refine_castle.camera_contract()
    terrain_before=refine_castle.lane_hash()
    ...
    export_scenes.export_static('backgrounds','castle','player',args.samples)
    bpy.context.view_layer.update()
    checks={'architecture_geometry_unchanged':geometry_before==signature(),
            'camera_unchanged':camera_before==refine_castle.camera_contract(),
            'terrain_unchanged':terrain_before==refine_castle.lane_hash(),
            ...
            'archive_hidden':bpy.data.collections['CAS_SourceArchive'].hide_render,
    ...
    report={'status':'passed','input':str(source),'input_sha256':original_hash,
            'input_is_canonical':source==ROOT/'art/blender/background-castle.blend',
            'isolated_source':str(prepared),'exporter':'tools/blender/export_scenes.py::export_static',
            'output':str(outdir/'exports/backgrounds/castle.png'),
    ...
    print('CASTLE_EXPORT_VALIDATED '+json.dumps(checks))
