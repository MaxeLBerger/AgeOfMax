...
for unit in ['archer','slinger','grenadier']:
    for faction in ['player','enemy']:
        ...
        print('weapon_parts',[(name,obj['location'],obj['groups']) for name,obj in objects.items() if any(word in name.lower() for word in ['bow','string','arrow','sling','stone','pouch','grenade','hand','boot','sole'])])
