...
ROOT=BUILD.ROOT
STAGE=BUILD.ROOT/'art/blender/candidates/animation-v3/ranged-family-2'
...
def main():
    ...
    for unit in ['musketeer','laser-soldier','plasma-trooper']:
        for faction in ['player','enemy']:
            ...
            pose_signatures={'walk':set(),'attack':set()}
            ...
            for clip in ['walk','attack']:
                ...
                for frame in range(8):
                    evaluate(clip,frame)
                    pose_signatures[clip].add(tuple(round(value,5) for name in ['head','forearm.near','forearm.far','shin.near','shin.far','weapon.rifle'] for row in rig.pose.bones[name].matrix for value in row))
                    vertices=[];polygons=[]
                    ...
                    if clip=='attack' and frame in [2,3,4,5]:
                        delta=rig.pose.bones['weapon.rifle'].matrix@rig.data.bones['weapon.rifle'].matrix_local.inverted()
                        sight_rest=Vector((.26,-.25,2.14) if unit=='sniper' else (.22,-.35,1.37))
            ...
            record={'passed':False,'samples_per_clip':129,'mesh_count':len(meshes),'bone_count':len(rig.data.bones),
                    ...
                    'unique_evaluated_walk_poses':len(pose_signatures['walk']),'unique_evaluated_attack_poses':len(pose_signatures['attack']),'eye_to_sight_axis_target_max_error_world':aim_error,'production_unchanged':current==model['source_hashes']}
            results[key]=record
            ...
            assert all(len(values)==8 for values in pose_signatures.values()),('duplicated geometry pose',key)
