...
test('Boot loads the complete five-epoch texture contract and sixteen real frames per troop', async ({ page }) => {
  await openMenu(page);
  ...
  for (const unit of state.units) {
    ...
    expect([unit.width, unit.height]).toEqual([4096, 256]);
    expect(unit.frames).toHaveLength(16);
  ...
  for (const { id } of units) {
    expect(state.sockets[id]).toHaveLength(16);
