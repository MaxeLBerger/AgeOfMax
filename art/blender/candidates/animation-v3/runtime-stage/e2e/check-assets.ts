...
export default function checkAssets(): void {
  ...
  for (const file of required.filter(file => file.endsWith('.png'))) {
    ...
    const expected = file.startsWith('units') ? [4096, 256] : file.startsWith('backgrounds') ? [1600, 900] : file.startsWith('bases') ? [512, 512] : [256, 256];
  ...
  parseGaitMetadata(JSON.parse(readFileSync(resolve('public/assets/reborn/gait-metadata.json'), 'utf8')));
  const sockets = JSON.parse(readFileSync(resolve('public/assets/reborn/weapon-sockets.json'), 'utf8')) as Record<string, number[][]>;
  for (const { id } of units) {
    if (!Array.isArray(sockets[id]) || sockets[id].length !== 16
      || sockets[id].some(point => !Array.isArray(point) || point.length !== 2 || point.some(value => !Number.isFinite(value)))) {
      throw new Error('weapon-sockets.json must contain sixteen finite [x, y] points for ' + id + '.');
    }
