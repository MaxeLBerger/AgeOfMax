...
export class BattleScene extends Phaser.Scene {
  ...
  private attackUnit(attacker: Phaser.Physics.Arcade.Sprite, target: Phaser.Physics.Arcade.Sprite): void {
    ...
    attacker.setData('attackUntil', this.simulationTime + UNIT_ANIMATION_LAYOUT.attackDurationMs);
    const attackerUid = attacker.getData('uid'), targetUid = target.getData('uid');
    // Eight attack samples retain the existing 320ms clip and 160ms contact.
    this.time.delayedCall(UNIT_ANIMATION_LAYOUT.contactDelayMs, () => {
  ...
  private attackBase(unit: Phaser.Physics.Arcade.Sprite, targetBaseSide: 'player' | 'enemy'): void {
    ...
    unit.setData('attackUntil', this.simulationTime + UNIT_ANIMATION_LAYOUT.attackDurationMs);
  ...
  private fireUnitProjectile(shooter: Phaser.Physics.Arcade.Sprite, target: Phaser.Physics.Arcade.Sprite): void {
    ...
    const muzzle = muzzlePoint(sockets, data.id, UNIT_ANIMATION_LAYOUT.contactFrame, shooter.x, shooter.y, shooter.scaleX ?? this.getUnitScale(data),
