...
describe('Blender weapon attachment', () => {
  it('projects a contact-frame muzzle around the foot anchor for both factions', () => {
    const frames: Array<[number, number]> = Array.from({ length: 16 }, () => [200, 176]);
    frames[12] = [220, 160];
    const sockets = { rifleman: frames };
    expect(muzzlePoint(sockets, 'rifleman', 12, 600, 507, .5, .5, false)).toEqual({ x: 646, y: 469.24 });
    expect(muzzlePoint(sockets, 'rifleman', 12, 600, 493, .5, .5, true)).toEqual({ x: 554, y: 455.24 });
  });
